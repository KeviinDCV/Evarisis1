...
- ✅ Resuelve: IHQ250995 no extraía "PROLIFERACIÓN ACINAR ATÍPICA (ASAP)" del bloque M250332
- ✅ Patrón -2.7 en extract_diagnostico_coloracion(): "bloque M[NUM]... con diagnóstico de..."
- ✅ FIX unified_extractor: Soportar "Estudios de inmunohistoquímica:" (dos puntos en lugar de punto)
- ✅ FIX unified_extractor: Patrón para capturar diagnóstico SIN guión inicial
...
def extract_diagnostico_coloracion(text: str) -> str:
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # Caso: Diagnóstico del estudio M previo citado entre comillas en formato:
    # 'diagnóstico "NEOPLASIA FUSOCELULAR A CLASIFICAR CON ESTUDIOS DE INMUNOHISTOQUÍMICA".'
    # Este aparece ANTES de la sección "INFORME PRELIMINAR"
    # Patrón: diagn[óo]stico "TEXTO". antes de INFORME PRELIMINAR

    patron_diagnostico_entrecomillado = r'diagn[óo]stico\s*\n\s*"([^"]+)"\s*\.\s*(?=.*?INFORME\s+PRELIMINAR)'
    match_diagnostico_entrecomillado = re.search(patron_diagnostico_entrecomillado, text, re.IGNORECASE | re.DOTALL)

    if match_diagnostico_entrecomillado:
        diagnostico_previo = match_diagnostico_entrecomillado.group(1).strip()
        # Normalizar espacios y saltos de línea (el texto puede estar fragmentado por OCR)
        diagnostico_previo = ' '.join(diagnostico_previo.split())
        # Convertir a mayúsculas para consistencia
        diagnostico_previo = diagnostico_previo.upper()
        # Validar que tiene contenido significativo
        if len(diagnostico_previo) > 10:
    ...
    if match_bloque_m:
        diagnostico_m = match_bloque_m.group(1).strip()
        # Normalizar espacios y saltos de línea
        diagnostico_m = ' '.join(diagnostico_m.split())
        # Convertir a mayúsculas para consistencia
        diagnostico_m = diagnostico_m.upper()
    ...
    # V6.1.4: FIX IHQ251000 - Patrón más simple sin comillas ni código M
    # Caso: "bloque 1 que corresponde a biopsia de parenquima hepatico con diagnostico de LOS HALLAZGOS..."
    # Patrón: "[bloque/estudio N] que corresponde a [descripción] con diagnostico de [DIAGNOSTICO]"
    patron_bloque_simple = r'(?:bloque|estudio)\s+\d+\s+que\s+corresponde\s+a\s+([^\s]+(?:\s+[^\s]+){0,10}?)\s+con\s+diagn[óo]stico\s+de\s+(.+?)(?=\.|$)'
    match_bloque_simple = re.search(patron_bloque_simple, text, re.IGNORECASE | re.DOTALL)

    if match_bloque_simple:
        diagnostico_m = match_bloque_simple.group(2).strip()
        # Normalizar espacios y saltos de línea
        diagnostico_m = ' '.join(diagnostico_m.split())
        # Convertir a mayúsculas para consistencia
        diagnostico_m = diagnostico_m.upper()
    ...
    # V6.1.2: Mejorado para capturar texto completo sin fragmentación (IHQ250997)
    # Termina en "Fecha de diagnóstico" en lugar de esperar doble salto de línea
    patron_diagnostico_inicial = r'Diagn[óo]stico\s+Inicial:\s*(.+?)(?=\s*\n\s*Fecha\s+de\s+diagn[óo]stico|DESCRIPCI[ÓO]N\s+MICROSC[ÓO]PICA|DIAGN[ÓO]STICO|Tras\s+la\s+valoraci[óo]n|$)'
    ...
    if match_diagnostico_inicial:
        ...
        # V6.1.2: Limpiar ". Fecha de" al final si quedó capturado (IHQ250997)
        diagnostico_inicial = re.sub(r'\.\s*Fecha\s+de\s*$', '', diagnostico_inicial, flags=re.IGNORECASE).strip()
...
def extract_factor_pronostico(diagnostico_completo: str, ihq_estudios_solicitados: str = "",
    ...
    Args:
        ...
        nombre_paciente: Nombre completo del paciente para filtrar contaminación (opcional)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # V6.0.4: LIMPIEZA FINAL - Filtrar contaminación de metadata
    # FIX: Ahora se pasa nombre_paciente para filtrar correctamente
    # ═══════════════════════════════════════════════════════════════════════
    factores_limpios = []
    for factor in factores:
        factor_limpio = limpiar_factor_pronostico(factor, nombre_paciente)
...
def extract_biomarcadores_solicitados_robust(text: str) -> List[str]:
    ...
    if not biomarcadores_encontrados:
        ...
        # Patrón 10: V6.0.4 - "para marcación de" (IHQ250982)
        # FIX: Eliminado '?' non-greedy para capturar lista completa hasta el punto final
...
def normalize_biomarker_name(name: str) -> str:
    ...
    # V6.1.3: CK5/6 y Calponina (IHQ250999)
    if 'CK5/6' in nombre_upper or 'CK5 / 6' in nombre_upper or 'CK5 6' in nombre_upper or nombre_upper == 'CK56':
        return 'IHQ_CK5_6'

    if 'CALPONINA' in nombre_upper:
        return 'IHQ_CALPONINA'
...
def extract_medical_data(text: str) -> Dict[str, Any]:
    ...
    # CORREGIDO v6.0.4: Extraer factor pronóstico con filtro de nombre del paciente
    # Extraer nombre del paciente para filtrar contaminación
    nombre_match = re.search(r'Nombre\s*:\s*([A-Z\s]+?)(?:\n|N\.\s)', clean_text, re.IGNORECASE)
    nombre_paciente = nombre_match.group(1).strip() if nombre_match else ""
...
_ORGN_PROC_STRIP = re.compile(
    r'(?i)\b(?:BX(?:\s+DE)?|BIOPSIA(?:\s+(?:POR|CON|DE|ENDOSC\w+|INCISIONAL|ESCISIONAL|PERCUT\w+|'
...
_ORGN_CANON = [
    (r'M[EÉ]DULA\s+[OÓ]SEA', 'MEDULA OSEA'),  # ANTES que OSEA->HUESO (médula ósea != hueso)
...
def extract_ihq_organ_from_diagnosis(diagnostico: str) -> str:
    ...
    V6.2.1: MEJORADO - Detecta diagnóstico FINAL vs preliminar
    - Si hay "DIAGNÓSTICO (FINAL)" o "Expansión", usa ese bloque
    - Evita capturar diagnósticos del estudio M previo
    ...
    Ejemplos:
        ...
        "Fémur derecho. Tumor. Biopsia por curetaje. Estudio de inmunohistoquímica. Expansión." → "FEMUR DERECHO"

    Args:
        diagnostico: Texto del diagnóstico completo (puede incluir preliminar + final)
    ...
    diagnostico_final_match = re.search(
        r'(?:DIAGN[ÓO]STICO\s*\(FINAL\)|Expansi[óo]n)[.\s]*([^\n]+\.\s*[^\n]+\.\s*[^\n]+\.)',
        text,
        re.IGNORECASE | re.DOTALL
    )

    if diagnostico_final_match:
    ...
    # → Queremos solo "Fémur derecho"
    text = re.sub(
        r'\.\s*(Estudio\s+de\s+inmunohistoqu[íi]mica|Expansi[óo]n).*$',
        '',
        text,
        flags=re.IGNORECASE
    )
    ...
    if match:
        ...
        procedimiento_terms = ['tumor', 'biopsia', 'resección', 'reseccion', 'extirpación', 'extirpacion', 'curetaje']
        ...
        if not any(candidato_lower == term for term in procedimiento_terms):
            ...
            # V6.2.1: Normalizar nombres de huesos y órganos comunes
            candidato_normalized = candidato.upper()

            # Validar que tenga longitud razonable (3-80 caracteres)
            if 3 <= len(candidato) <= 80:
                return candidato_normalized
        ...
        for keyword in keywords:
            if keyword.lower() in text.lower():
                ...
                if idx >= 0:
                    ...
                    if seg_match:
                        return seg_match.group(1).strip().upper()
    ...
    if len(palabras) >= 2:
        ...
        for i, palabra in enumerate(palabras[:6]):
            if palabra.lower() in ['tumor', 'biopsia', 'resección', 'carcinoma', 'adenocarcinoma']:
                if i > 0:
                    return ' '.join(palabras[:i]).strip('. ').upper()
