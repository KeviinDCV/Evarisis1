...
log_file = get_base_path() / 'debug_psa.log'
logging.basicConfig(
    level=logging.INFO,
...
NEW_TABLE_COLUMNS_ORDER: List[str] = [
    ...
    "IHQ_RACEMASA", "IHQ_34BETA", "IHQ_B2",
...
def _create_table_if_not_exists(cursor: sqlite3.Cursor):
    ...
    cursor.execute(f"""
        CREATE TABLE IF NOT EXISTS {TABLE_NAME} (
            ...
            "IHQ_RACEMASA" TEXT, "IHQ_34BETA" TEXT, "IHQ_B2" TEXT,
...
def _add_new_biomarker_columns(conn: sqlite3.Connection, cursor: sqlite3.Cursor):
    ...
    new_biomarkers = [
        ...
        "IHQ_CK7", "IHQ_DESMINA", "IHQ_LCA", "IHQ_CD11", "IHQ_MIOGENINA", "IHQ_MAMAGLOBINA", "IHQ_TIROGLOBULINA", "IHQ_CK34BETAE12", "IHQ_CK34BETA12", "IHQ_OCT4", "IHQ_PODOPLANINA", "IHQ_IDH", "IHQ_GPC3", "IHQ_AFP", "IHQ_IGD", "IHQ_BETACATENINA", "IHQ_ACTINA_MUSCULO_ESPECIFICA", "IHQ_MIELOPEROXIDASA", "IHQ_CD7", "IHQ_HCG", "IHQ_ACTINA_MUSCULO_LISO", "IHQ_EBER", "IHQ_CALRRETININA", "IHQ_SINAPTOFISINA", "IHQ_CROMOGRANINA", "IHQ_CK56", "IHQ_CAM5", "IHQ_GLICOFORINA", "IHQ_TDT", "IHQ_ATRX", "IHQ_IDH1", "IHQ_CMYC", "IHQ_IGG4", "IHQ_IGG", "IHQ_MAMOGLOBINA", "IHQ_HEPATOCITO", "IHQ_PSA", "IHQ_RCC", "IHQ_CK19", "IHQ_CK20", "IHQ_CDX2", "IHQ_EMA", "IHQ_GATA3", "IHQ_SOX10", "IHQ_SOX11",
        "IHQ_KAPPA",
        "IHQ_LAMBDA",
    ...
    # V6.0.20: Mejorado para NO sobrescribir valores válidos existentes
    if columns_to_fill:
        for biomarker in columns_to_fill:
            ...
            if biomarker != "Estado Auditoria IA":
                try:
                    # Verificar si ya existen valores válidos en esta columna
                    cursor.execute(
                        f'SELECT COUNT(*) FROM {TABLE_NAME} WHERE "{biomarker}" IS NOT NULL AND "{biomarker}" != "" AND "{biomarker}" != "N/A"'
                    )
                    count_existing = cursor.fetchone()[0]

                    # FIX CRÍTICO V6.2.0: SIEMPRE verificar que NO sobrescribimos valores válidos
                    # El problema: si count_existing==0 PERO se acaban de insertar valores (sin commit),
                    # el UPDATE puede sobrescribir esos valores recién insertados
                    # SOLUCIÓN: Solo actualizar registros que tengan NULL o "" (nunca otros valores)
                    cursor.execute(
                        f'UPDATE {TABLE_NAME} SET "{biomarker}" = ? WHERE "{biomarker}" IS NULL OR "{biomarker}" = ""',
                        ('N/A',)
                    )
                    rows_updated = cursor.rowcount
                    if rows_updated > 0:
                        logger.info(f"🔄 Rellenado '{biomarker}' con 'N/A' en {rows_updated} registros vacíos")

                    # Log si había valores válidos previos
                    if count_existing > 0:
                        logger.info(f"✓ '{biomarker}' ya tiene {count_existing} valores válidos (preservados)")
                except sqlite3.OperationalError as e:
...
def _normalize_column_name(column_name: str) -> str:
    ...
    """
    # V6.1.4: FIX CRÍTICO - Columnas con prefijo IHQ_ deben retornar tal cual (sin normalizar)
    # Problema: unified_extractor.py genera claves como 'IHQ_HEPATOCITO' (mayúsculas)
    # pero _normalize_column_name() las convertía a 'ihq_hepatocito' (minúsculas)
    # causando que save_records() no encuentre el valor en el diccionario record
    if column_name.startswith('IHQ_'):
        return column_name  # Retornar sin modificar
    ...
    try:
        ...
        for record in records:
            ...
            try:
                ...
                # DEBUG V6.0.17: Log para diagnosticar PSA
                if peticion == 'IHQ250996':
                    psa_keys = [k for k in record.keys() if 'PSA' in k.upper()]
                    logger.info(f"DEBUG IHQ250996 - Claves con PSA ANTES de _apply_default_values: {psa_keys}")
                    for key in psa_keys:
                        logger.info(f"  {key}: {record.get(key)}")

                # DEBUG: Log para IHQ251000 ANTES de _apply_default_values
                if peticion == 'IHQ251000':
                    logger.info(f"[{peticion}] ANTES de _apply_default_values:")
                    logger.info(f"  'IHQ_HEPATOCITO' en record.keys(): {'IHQ_HEPATOCITO' in record}")
                    logger.info(f"  record.get('IHQ_HEPATOCITO'): {repr(record.get('IHQ_HEPATOCITO', 'KEY_NOT_FOUND'))}")
                    if 'IHQ_HEPATOCITO' in record:
                        val = record['IHQ_HEPATOCITO']
                        logger.info(f"  Valor: {repr(val)}, type: {type(val)}, bool: {bool(val)}, str().strip(): '{str(val).strip()}'")

                # Aplicar valores por defecto N/A a campos vacíos
                record = _apply_default_values(record)

                # DEBUG V6.0.17: Log DESPUÉS de _apply_default_values
                if peticion == 'IHQ250996':
                    logger.info(f"DEBUG IHQ250996 - IHQ_PSA DESPUÉS de _apply_default_values: {record.get('IHQ_PSA')}")

                # DEBUG: Log para IHQ251000 DESPUÉS de _apply_default_values
                if peticion == 'IHQ251000':
                    logger.info(f"[{peticion}] DESPUES de _apply_default_values:")
                    logger.info(f"  record.get('IHQ_HEPATOCITO'): {repr(record.get('IHQ_HEPATOCITO', 'KEY_NOT_FOUND'))}")
                ...
                if existing:
                    ...
                    for col in table_columns:
                        ...
                        # DEBUG: Log detallado para HEPATOCITO en INSERT
                        if col == 'IHQ_HEPATOCITO':
                            logger.info(f"[INSERT {peticion}] IHQ_HEPATOCITO: normalized='{normalized_col}', record.get(normalized)={repr(record.get(normalized_col, 'KEY_NOT_FOUND'))}, record.get(col)={repr(record.get(col, 'KEY_NOT_FOUND'))}, final_value={repr(value)}")
                            # Log todas las claves IHQ_* en record
                            ihq_keys = [k for k in record.keys() if k.startswith('IHQ_')]
                            logger.info(f"[INSERT {peticion}] Total claves IHQ_* en record: {len(ihq_keys)}")
                            if 'IHQ_HEPATOCITO' in record:
                                logger.info(f"[INSERT {peticion}] record['IHQ_HEPATOCITO'] = {repr(record['IHQ_HEPATOCITO'])}")
                            else:
                                logger.warning(f"[INSERT {peticion}] 'IHQ_HEPATOCITO' NO encontrado en record keys")
