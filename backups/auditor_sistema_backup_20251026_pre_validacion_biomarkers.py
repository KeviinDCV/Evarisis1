...
Versión: 3.0.1 (MEJORAS VALIDACIÓN)
...
class AuditorSistema:
    ...
    def _validar_diagnostico_coloracion(self, bd: Dict, ocr: str) -> Dict:
        ...
        MEJORA v6.0.9: Busqueda robusta en TODO el OCR con normalizacion avanzada
        ...
        # Normalizar valor BD (eliminar espacios multiples, saltos de linea, comillas)
        valor_bd_norm = re.sub(r'\s+', ' ', valor_bd).strip().upper()
        valor_bd_norm = valor_bd_norm.replace('"', '').replace("'", '')

        # Normalizar OCR completo
        ocr_norm = re.sub(r'\s+', ' ', ocr).upper()

        # ESTRATEGIA 1: Busqueda exacta del valor BD normalizado en OCR
        if valor_bd_norm in ocr_norm:
            return {'estado': 'OK', 'valor': valor_bd, 'mensaje': 'Correcto (busqueda exacta)'}

        # ESTRATEGIA 2: Busqueda por componentes principales
        # Extraer componentes clave del diagnostico (sin puntuacion ni conectores)
        componentes = [c.strip() for c in re.split(r'[,.:;/]', valor_bd_norm) if c.strip() and len(c.strip()) > 3]
        componentes_encontrados = sum(1 for comp in componentes if comp in ocr_norm)

        # Si encontramos >= 80% de los componentes, consideramos valido
        if componentes and (componentes_encontrados / len(componentes)) >= 0.8:
            return {
                'estado': 'OK',
                'valor': valor_bd,
                'mensaje': f'Correcto (validacion por componentes: {componentes_encontrados}/{len(componentes)})'
            }

        # ESTRATEGIA 3: Busqueda por patron flexible en descripcion macroscopica
        patron = r'diagn[óo]stico\s+de\s*["\']?(.+?)["\']?[\.\n]'
        match = re.search(patron, ocr, re.IGNORECASE | re.DOTALL)

        if match:
            valor_ocr = match.group(1).strip()
            valor_ocr_norm = re.sub(r'\s+', ' ', valor_ocr).strip().upper()

            # Comparacion normalizada
            if valor_bd_norm == valor_ocr_norm:
                return {'estado': 'OK', 'valor': valor_bd, 'mensaje': 'Correcto (patron descripcion)'}

        # Si llegamos aqui, no pudimos validar
        return {
            'estado': 'WARNING',
            'valor': valor_bd,
            'mensaje': 'DIAGNOSTICO_COLORACION no pudo ser validado en OCR (verificar manualmente)',
            'nota': 'Este puede ser un falso positivo si el diagnostico esta presente pero con formato diferente'
        }
