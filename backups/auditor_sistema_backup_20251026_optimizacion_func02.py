...
class AuditorSistema:
    ...
    def corregir_caso_automatico(self, numero_caso: str, auto_aprobar: bool = False, max_iteraciones: int = 3) -> Dict:
        """
        FUNC-02: Corrección automática iterativa con validación

        Flujo simplificado:
        1. Audita con FUNC-01
        2. Si hay errores, aplica correcciones automáticas
        3. Reprocesa PDF
        4. Re-audita
        5. Itera hasta max_iteraciones si necesario
        """
        print(f"\n{'='*80}")
        ...
        for iteracion in range(1, max_iteraciones + 1):
            ...
            # PASO 1: Auditar con FUNC-01 (completo)
            print("  1️⃣ Auditando con FUNC-01...")
            resultado = self.auditar_caso_inteligente(numero_caso, json_export=False, nivel='completo')

            if not resultado:
                print("  ❌ No se encontró debug_map. Saliendo.")
                return {"estado": "ERROR", "mensaje": "Debug map no encontrado"}
            ...
            # PASO 2: Aplicar correcciones simples
            print(f"  2️⃣ Aplicando correcciones automáticas...")
            cambios_aplicados = self._aplicar_correcciones_simples(resultado, auto_aprobar)
    ...
    def _aplicar_correcciones_simples(self, resultado: Dict, auto_aprobar: bool) -> bool:
        """
        Aplica correcciones automáticas basadas en errores detectados.

        Correcciones soportadas:
        1. DIAGNOSTICO_COLORACION: Corrige valor en BD desde OCR
        2. FACTOR_PRONOSTICO: Corrige a "NO APLICA" si no hay biomarcadores FP
        ...
        if not errores:
            print(f"     ✅ No hay errores que corregir")
            return False
        ...
        datos_bd = resultado.get('_datos_bd', {})
        ...
        if diag_coloracion.get('estado') == 'ERROR':
            valor_correcto = diag_coloracion.get('valor_ocr', '')
            if valor_correcto:
                print(f"\n     🔧 Corrección 1: DIAGNOSTICO_COLORACION")
                print(f"        Valor actual: {diag_coloracion.get('valor_bd', 'N/A')}")
                print(f"        Valor correcto: {valor_correcto}")

                if auto_aprobar or self._confirmar_correccion("DIAGNOSTICO_COLORACION", diag_coloracion.get('valor_bd'), valor_correcto):
                    if self._actualizar_bd(numero_caso, 'Diagnostico Coloracion', valor_correcto):
                        correcciones_aplicadas.append('DIAGNOSTICO_COLORACION')
                        print(f"        ✅ Corregido")

        # Corrección 2: FACTOR_PRONOSTICO
        factor_pron = auditoria_bd.get('factor_pronostico', {})
        if factor_pron.get('estado') == 'ERROR':
            mensaje = factor_pron.get('mensaje', '')
            if 'debe ser "NO APLICA"' in mensaje:
                print(f"\n     🔧 Corrección 2: FACTOR_PRONOSTICO")
                print(f"        Valor actual: {factor_pron.get('valor_bd', 'N/A')}")
                print(f"        Valor correcto: NO APLICA")

                if auto_aprobar or self._confirmar_correccion("FACTOR_PRONOSTICO", factor_pron.get('valor_bd'), "NO APLICA"):
                    if self._actualizar_bd(numero_caso, 'Factor pronostico', 'NO APLICA'):
                        correcciones_aplicadas.append('FACTOR_PRONOSTICO')
                        print(f"        ✅ Corregido")

        if correcciones_aplicadas:
            print(f"\n     ✅ {len(correcciones_aplicadas)} corrección(es) aplicada(s): {', '.join(correcciones_aplicadas)}")
            return True
        else:
            print(f"\n     ⚠️ No se aplicaron correcciones automáticas")
            return False

    def _confirmar_correccion(self, campo: str, valor_actual: str, valor_nuevo: str) -> bool:
        """Solicita confirmación al usuario para aplicar corrección"""
        print(f"\n     ❓ ¿Aplicar corrección a {campo}?")
        print(f"        Actual: {valor_actual}")
        print(f"        Nuevo:  {valor_nuevo}")
        respuesta = input("        (s/n): ").strip().lower()
        return respuesta in ['s', 'si', 'y', 'yes']

    def _actualizar_bd(self, numero_caso: str, campo: str, valor: str) -> bool:
        ...
        try:
            ...
            backup_path = self.project_root / "backups" / f"bd_backup_{numero_caso}_{timestamp}.db"
            ...
            # Actualizar campo
            query = f"UPDATE {self.table_name} SET `{campo}` = ? WHERE `Numero de caso` = ?"
            cursor.execute(query, (valor, numero_caso))
            conn.commit()
            conn.close()
    ...
    def _reprocesar_caso(self, numero_caso: str) -> bool:
        ...
        3. Ejecuta main.py con el PDF completo
        4. Espera hasta 5 minutos para completar
        ...
        try:
            ...
            # PASO 3: Procesar PDF completo
            print(f"\n     🔄 Procesando PDF completo...")
            print(f"        PDF: {pdf_path.name}")
            print(f"        Comando: main.py \"{pdf_path}\"")

            main_script = self.project_root / "main.py"
            cmd = f'python "{main_script}" "{pdf_path}"'
            ...
            if result.returncode == 0:
                ...
                print(f"        Verificar nuevo debug_map en: data/debug_maps/")
                return True
            else:
                print(f"\n     ❌ Error en procesamiento (código: {result.returncode})")
                if result.stderr:
            ...
            print(f"\n     ❌ Timeout: El procesamiento tardó más de 5 minutos")
            return False
        except Exception as e:
            print(f"\n     ❌ Excepción durante reprocesamiento:")
            print(f"        {str(e)[:200]}")
            import traceback
