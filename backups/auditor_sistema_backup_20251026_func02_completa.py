...
class AuditorSistema:
    ...
    def _validar_sintaxis_python(self, archivo: Path) -> bool:
        ...
        try:
            # Compilar a archivo temporal
            with tempfile.NamedTemporaryFile(suffix='.pyc', delete=True) as tmp:
                py_compile.compile(str(archivo), cfile=tmp.name, doraise=True)
            return True
...
'''
                ...
                if indice_insercion:
                    ...
                    # Ahora agregar lógica de filtrado ANTES del return
                    # Buscar el return que devuelve la lista de factores
                    for i, linea in enumerate(lineas_funcion):
                        if 'return' in linea and 'factores' in linea:
                            # Insertar filtro ANTES del return
                            filtro_aplicacion = '''    # Filtrar biomarcadores de tipificación
    factores_filtrados = []
