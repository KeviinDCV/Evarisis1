...
def extract_factor_pronostico(diagnostico_completo: str) -> str:
    ...
    Returns:
        Factor pronóstico extraído con todos los componentes encontrados
    """
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # Buscar patrones como:
    # - "NOTTINGHAM GRADO 2 (PUNTAJE DE 6)"
    # - "Grado histológico Nottingham: 3"
    ...
    for pattern in nottingham_patterns:
        match = re.search(pattern, diagnostico_completo, re.IGNORECASE)
        if match:
            grado = match.group(1)
            puntaje = match.group(2) if len(match.groups()) > 1 and match.group(2) else None
            if puntaje:
                nottingham_text = f"Grado Nottingham {grado} (puntaje {puntaje})"
            else:
                nottingham_text = f"Grado Nottingham {grado}"
            factores_encontrados.append(nottingham_text)
            break
    ...
    for pattern in linfovascular_patterns:
        match = re.search(pattern, diagnostico_completo, re.IGNORECASE)
        if match:
            estado = match.group(1).upper()
            # Normalizar el estado
            if 'PRESENTE' in estado or estado in ('SÍ', 'SI'):
                linfo_text = "Invasión linfovascular: PRESENTE"
            elif 'NO' in estado or 'AUSENTE' in estado or 'NEGATIV' in estado:
                linfo_text = "Invasión linfovascular: ausente"
            else:
                linfo_text = f"Invasión linfovascular: {estado}"
            factores_encontrados.append(linfo_text)
            break
    ...
    for pattern in perineural_patterns:
        match = re.search(pattern, diagnostico_completo, re.IGNORECASE)
        if match:
            estado = match.group(1).upper()
            # Normalizar el estado
            if 'PRESENTE' in estado or estado in ('SÍ', 'SI'):
                peri_text = "Invasión perineural: PRESENTE"
            elif 'NO' in estado or 'AUSENTE' in estado or 'NEGATIV' in estado:
                peri_text = "Invasión perineural: ausente"
            else:
                peri_text = f"Invasión perineural: {estado}"
            factores_encontrados.append(peri_text)
            break
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # Buscar patrones como:
    # - "CARCINOMA DUCTAL IN SITU NO IDENTIFICADO"
    ...
    for pattern in dcis_patterns:
        match = re.search(pattern, diagnostico_completo, re.IGNORECASE)
        if match:
            estado = match.group(1).upper()
            # Normalizar el estado
            if 'PRESENTE' in estado:
                dcis_text = "Carcinoma ductal in situ: PRESENTE"
            elif 'NO' in estado or 'AUSENTE' in estado or 'NEGATIV' in estado:
                dcis_text = "Carcinoma ductal in situ: ausente"
            else:
                dcis_text = f"Carcinoma ductal in situ: {estado}"
            factores_encontrados.append(dcis_text)
            break

    # ═══════════════════════════════════════════════════════════════════════
    # PRIORIDAD 7: Líneas de inmunorreactividad en descripción microscópica
    # ═══════════════════════════════════════════════════════════════════════
    # Buscar patrones como:
