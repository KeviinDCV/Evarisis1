...
Versión: 3.1.2 (LIMPIEZA DIAGNOSTICO_PRINCIPAL)
Fecha: 29 de octubre de 2025

CHANGELOG v3.1.2 - LIMPIEZA DIAGNOSTICO_PRINCIPAL:
- ✅ NUEVA FUNCIÓN: _limpiar_diagnostico_principal() remueve metadatos
- 🧹 MEJORA: Extrae SOLO el diagnóstico principal sin órgano, tipo de muestra, tipo de estudio
- 🧹 MEJORA: Remueve frases introductorias ("LOS HALLAZGOS MORFOLÓGICOS Y DE... SON COMPATIBLES CON")
- 🎯 MEJORA: Patrones de limpieza para 5 tipos de metadatos comunes
- ✅ VALIDADO: IHQ250992 extrae "NEOPLASIA DE CÉLULAS PLASMÁTICAS..." (sin "Médula ósea. Biopsia.")
- 📝 Backup: auditor_sistema_backup_20251029_pre_v3.1.2.py

CHANGELOG v3.1.1 - LOGGING MEJORADO:
- 📝 MEJORA: Logging detallado en _obtener_debug_map() con emojis visuales
- ✅ MEJORA: Reporta ruta completa del debug_map encontrado
- ✅ MEJORA: Valida estructura básica del debug_map (OCR + BD presentes)
- 🔍 MEJORA: Mensajes más claros cuando debug_map no se encuentra
- 📁 MEJORA: Muestra directorio de búsqueda en logs
- 📝 Backup: auditor_sistema_backup_20251029_pre_v3.1.1.py
...
class AuditorSistema:
    ...
    def _obtener_debug_map(self, numero_caso: str) -> Optional[Dict]:
        ...
        self.logger.info(f"🔍 Buscando debug_map con patrón: {patron}")
        self.logger.info(f"📁 Directorio de búsqueda: {self.debug_maps_dir}")

        archivos = list(self.debug_maps_dir.glob(patron))

        if not archivos:
            self.logger.error(f"❌ No se encontró debug_map para {numero_caso}")
            self.logger.error(f"   Patrón buscado: {patron}")
            self.logger.error(f"   Directorio: {self.debug_maps_dir}")
            return None

        archivo_mas_reciente = max(archivos, key=lambda p: p.stat().st_mtime)
        self.logger.info(f"✅ Debug_map encontrado: {archivo_mas_reciente.name}")
        self.logger.info(f"   Ruta completa: {archivo_mas_reciente}")

        try:
            with open(archivo_mas_reciente, 'r', encoding='utf-8') as f:
                data = json.load(f)
                self.logger.info(f"✅ Debug_map cargado exitosamente para {numero_caso}")

                # Validar estructura básica
                if 'ocr' in data and 'base_datos' in data:
                    self.logger.info(f"   ✓ Estructura válida: OCR y BD presentes")
                else:
                    self.logger.warning(f"   ⚠️  Estructura incompleta en debug_map")
    ...
    def _limpiar_diagnostico_principal(self, diagnostico: str) -> str:
        """
        Limpia DIAGNOSTICO_PRINCIPAL removiendo metadatos y frases introductorias.

        V3.1.2: Nueva función para extraer solo el diagnóstico sin metadatos.
        ...
        Args:
            diagnostico: Diagnóstico completo con metadatos

        Returns:
            Diagnóstico limpio (solo el diagnóstico principal)

        Complexity: CC ~6
        """
        # 1. Remover órgano + tipo de muestra al inicio
        # Patrones comunes: "Médula ósea. Biopsia.", "Mama derecha. Mastectomía.", etc.
        diagnostico = re.sub(r'^[^\.]+\.\s+[^\.]+\.\s+', '', diagnostico, flags=re.IGNORECASE)

        # 2. Remover tipo de estudio
        diagnostico = re.sub(r'^Estudios?\s+de\s+\w+\.\s+', '', diagnostico, flags=re.IGNORECASE)

        # 3. Remover frases introductorias comunes
        ...
        for patron in frases_introductorias:
            diagnostico = re.sub(patron, '', diagnostico, flags=re.IGNORECASE)

        # 4. Limpiar espacios y capitalizar correctamente
        diagnostico = diagnostico.strip()

        # 5. Si queda muy corto (< 10 chars), probablemente falló la limpieza
        if len(diagnostico) < 10:
            return diagnostico  # Devolver sin limpiar
    ...
    def _extraer_diagnostico_principal_desde_ocr(self, ocr: str) -> Optional[str]:
        ...
        V3.1.2: Mejorado para limpiar metadatos y extraer solo el diagnóstico principal.
        V3.1.0: Nueva función para evitar validación circular.

        Extrae el diagnóstico desde la sección "DIAGNÓSTICO" del OCR y limpia:
        - Órgano y tipo de muestra
        - Tipo de estudio (IHQ, coloración, etc.)
        ...
        Returns:
            Diagnóstico limpio (solo diagnóstico principal, sin metadatos)
        ...
        if match:
            ...
            # 🆕 LIMPIAR metadatos (órgano, tipo de muestra, frases introductorias)
            diagnostico = self._limpiar_diagnostico_principal(diagnostico)
    ...
    def _extraer_diagnostico_coloracion_desde_ocr(self, ocr: str) -> Optional[str]:
        ...
        V3.1.0: Nueva función para evitar validación circular.
        Busca diagnóstico del estudio M previo (bloque M + "diagnóstico de").
        ...
        patron = r'bloque\s+M\d+.*?diagn[óo]stico\s+de\s*["\'](.+?)["\']'
