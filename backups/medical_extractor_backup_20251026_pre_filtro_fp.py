...
Versión: 4.2.1 - Normalización FACTOR_PRONOSTICO (REGLAS DE ORO)
...
def extract_diagnostico_coloracion(text: str) -> str:
    # FUNC-02: Patrón para material extrainstitucional (IHQ250982)
    patron_material_externo = r'APARENTE\s+COMPROMISO.*?(?:NEOPLASIA|METASTÁSICA|MALIGNA)'
    match_material = re.search(patron_material_externo, text, re.IGNORECASE | re.DOTALL)
    if match_material:
...
def extract_factor_pronostico(diagnostico_completo: str, ihq_estudios_solicitados: str = "",
    ...
    # FUNC-02: Lista negra de biomarcadores de tipificación (NO son factores pronósticos)
    ...
    if listas_encontradas:
        # FUNC-02: Filtrar biomarcadores de tipificación de listas narrativas
        listas_filtradas = []
        for lista in listas_encontradas:
            contiene_tipificacion = False
            for bio_tip in biomarcadores_tipificacion:
                if bio_tip.upper() in lista.upper():
                    contiene_tipificacion = True
                    break
            if not contiene_tipificacion:
