...
class AuditorSistema:
    ...
    def _auditar_datos_guardados(self, resultado: Dict) -> None:
        ...
        if estado_coloracion['estado'] != 'OK':
        ...
        if match:
            ...
            if valor_bd_norm == valor_correcto:
                ...
                return {
                    ...
                    'mensaje': 'Diagnostico Coloracion no coincide exactamente'
                }
            ...
            return {
                'estado': 'WARNING',
                'valor': valor_bd,
                'mensaje': 'Diagnostico Coloracion presente pero no encontrado en OCR'
            }

    def _validar_diagnostico_principal(self, bd: Dict, ocr: str) -> Dict:
        """
        Valida DIAGNOSTICO_PRINCIPAL contra OCR.
        ...
        Complexity: CC ~6
        """
        valor_bd = bd.get('Diagnostico Principal', '')

        # Buscar primera linea significativa de seccion DIAGNOSTICO
        patron = r'DIAGN[ÓO]STICO\s*\n(?:.*?\.\s*){0,4}-?\s*(.+?)(?:\n|-|$)'
        ...
        if match:
            ...
            if valor_bd_norm in valor_correcto or valor_correcto in valor_bd_norm:
                return {'estado': 'OK', 'valor': valor_bd, 'mensaje': 'Correcto'}
            else:
                return {
                    'estado': 'ERROR',
                    'valor_bd': valor_bd,
                    'valor_ocr': valor_correcto,
                    'mensaje': 'Diagnostico Principal incorrecto'
                }
        else:
            return {
                'estado': 'WARNING',
                'valor': valor_bd,
                'mensaje': 'No se pudo extraer diagnostico del OCR'
            }
        ...
        if not presentes:
            ...
            if factor_bd in ['NO APLICA', 'N/A', '']:
                ...
                return {
                    'estado': 'WARNING',
                    'valor_bd': factor_bd,
                    'mensaje': 'Factor pronostico debe ser "NO APLICA" (no hay biomarcadores de FP)'
                }
