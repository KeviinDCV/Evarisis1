...
def extract_factor_pronostico(diagnostico_completo: str, ihq_estudios_solicitados: str = "",
    ...
    # Termina en marcadores claros: "Isquemia fría", "Todos los análisis", "FIRMA", "PATOLOGO"
    patron_bloque_amplio = r'(?:REPORTE\s+DE\s+BIOMARCADORES?|BIOMARCADORES?)\s*:?\s*(.+?)(?=\n\s*Isquemia\s+fría|Todos\s+los\s+an[aá]lisis|FIRMA|PATOLOGO|\Z)'
    ...
    if match_bloque:
        ...
        for match in matches_lineas:
            ...
            # V6.0.15: CRÍTICO - Normalizar nombres de biomarcadores comunes (IHQ250984)
            ...
            # Para Ki-67: capturar solo el porcentaje
            ...
            # Para ER/PR: capturar solo POSITIVO/NEGATIVO
            elif nombre_bio in ['ER', 'PR']:
                patron_er_pr = r'(POSITIVO|NEGATIVO|POSITIVA|NEGATIVA)'
                match_er_pr = re.search(patron_er_pr, valor_bio, re.IGNORECASE)
                if match_er_pr:
                    valor_bio = match_er_pr.group(1).upper()
                    # Normalizar femenino a masculino
                    valor_bio = valor_bio.replace('POSITIVA', 'POSITIVO').replace('NEGATIVA', 'NEGATIVO')
