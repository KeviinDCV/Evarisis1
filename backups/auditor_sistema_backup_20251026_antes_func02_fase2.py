    ...
    def _aplicar_correcciones_extractores(self, resultado: Dict, numero_caso: str, auto_aprobar: bool = False) -> bool:
        ...
        if diag_coloracion.get('estado') == 'ERROR':
            ...
            if 'APARENTE COMPROMISO' in valor_ocr.upper() or 'METASTÁSICA' in valor_ocr.upper():
                ...
                # VIRTUALIZAR CORRECCIÓN (buscar función y agregar patrón)
                # Buscar la función extract_diagnostico_coloracion
                import re

                # Patrón para encontrar la función
                patron_funcion = r'(def extract_diagnostico_coloracion\(.*?\):.*?(?=\n(?:def |class |$)))'
                match_funcion = re.search(patron_funcion, codigo_actual, re.DOTALL)

                if match_funcion:
                    funcion_actual = match_funcion.group(1)

                    # Buscar donde está el return
                    # Agregar patrón ANTES del return final
                    patron_material = '''        # Patrón para material extrainstitucional
        patron_material_externo = r'APARENTE\\s+COMPROMISO.*?(?:NEOPLASIA|METASTÁSICA|MALIGNA)'
        match_material = re.search(patron_material_externo, texto_completo, re.IGNORECASE | re.DOTALL)
        if match_material:
            diagnostico = match_material.group(0).strip()
            return diagnostico

'''

                    # Buscar el primer return en la función
                    lineas_funcion = funcion_actual.split('\n')
                    indice_return = None
                    for i, linea in enumerate(lineas_funcion):
                        if 'return' in linea and not linea.strip().startswith('#'):
                            indice_return = i
                            break

                    if indice_return:
                        # Insertar patrón ANTES del return
                        lineas_funcion.insert(indice_return, patron_material)
                        funcion_modificada = '\n'.join(lineas_funcion)

                        # Reemplazar en código completo
                        codigo_modificado = codigo_actual.replace(funcion_actual, funcion_modificada)
        ...
        if factor_pronostico.get('estado') == 'ERROR':
            ...
            # VIRTUALIZAR CORRECCIÓN (agregar filtro de tipificación)
            import re

            # Buscar la función extract_factor_pronostico
            patron_funcion = r'(def extract_factor_pronostico\(.*?\):.*?(?=\n(?:def |class |$)))'
            match_funcion = re.search(patron_funcion, codigo_actual, re.DOTALL)

            if match_funcion:
                funcion_actual = match_funcion.group(1)

                # Agregar lista negra al inicio de la función
                codigo_filtro = '''    # Lista negra de biomarcadores de tipificación (NO son factores pronósticos)
...
'''
                ...
                # Buscar donde comienza el cuerpo de la función (después de la docstring)
                lineas_funcion = funcion_actual.split('\n')
                indice_insercion = None

                for i, linea in enumerate(lineas_funcion):
                    # Buscar fin de docstring o primera línea de código
                    if '"""' in linea or "'''" in linea:
                        # Contar comillas para saber si cerramos docstring
                        if linea.count('"""') == 2 or linea.count("'''") == 2:
                            indice_insercion = i + 1
                            break
                    elif linea.strip() and not linea.strip().startswith('#') and i > 2:
                        indice_insercion = i
                        break

                if indice_insercion:
                    # Insertar código de filtro
                    lineas_funcion.insert(indice_insercion, codigo_filtro)

                    # Agregar lógica de filtrado ANTES del bloque de combinación final
                    # Buscar comentario "# V6.0.16: Combinar biomarcadores" como punto de anclaje
                    for i, linea in enumerate(lineas_funcion):
                        if '# V6.0.16: Combinar biomarcadores' in linea or '# Si NO hay estructurados' in linea:
                            # Insertar filtro ANTES de este comentario
                            filtro_aplicacion = '''    # Filtrar biomarcadores de tipificación ANTES de combinar
    factores_filtrados = []
    if 'factores_adicionales' in locals() and factores_adicionales:
        for factor in factores_adicionales:
            es_tipificacion = False
            for bio_tip in biomarcadores_tipificacion:
                if bio_tip.upper() in factor.upper():
                    es_tipificacion = True
                    break
            if not es_tipificacion:
                factores_filtrados.append(factor)
        factores_adicionales = factores_filtrados

'''
                            lineas_funcion.insert(i, filtro_aplicacion)
                            break
