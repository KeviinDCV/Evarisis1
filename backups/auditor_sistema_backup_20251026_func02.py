...
def main():
    """CLI principal - SOLO auditoría inteligente"""
    parser = argparse.ArgumentParser(
        description="🔍 Auditor EVARISIS - Auditoría Inteligente (FUNC-01)",
...
El resultado se exporta automáticamente a JSON en:
  herramientas_ia/resultados/auditoria_inteligente_IHQ250980.json
    ...
    parser.add_argument("--inteligente", action="store_true", required=True,
                        help="Auditoría inteligente (REQUERIDO)")

    args = parser.parse_args()

    if not args.inteligente:
        print("❌ ERROR: --inteligente es REQUERIDO")
        print("Uso: python auditor_sistema.py IHQ250980 --inteligente")
        sys.exit(1)
