...
class AuditorSistema:
    ...
    'CD1A': 'IHQ_CD1A', 'CD1a': 'IHQ_CD1A', 'CD-1A': 'IHQ_CD1A', 'CD-1a': 'IHQ_CD1A', 'CD 1A': 'IHQ_CD1A', 'CD 1a': 'IHQ_CD1A',  # V6.2.1: Timomas (IHQ251012)
    ...
    def _auditar_datos_guardados(self, resultado: Dict) -> None:
        ...
        # V6.2.2: Print detallado de biomarcadores NO MAPEADOS
        if estado_biomarcadores.get('no_mapeados'):
            print(f"\n⚠️ BIOMARCADORES NO MAPEADOS DETECTADOS:")
            print(f"   Total solicitado (OCR): {estado_biomarcadores.get('total_solicitado_ocr', 0)}")
            print(f"   Mapeados en BD: {estado_biomarcadores.get('mapeados', 0)}")
            print(f"   NO mapeados: {len(estado_biomarcadores.get('no_mapeados', []))}")
            print(f"\n   Lista de NO MAPEADOS:")
            for idx, bio in enumerate(estado_biomarcadores.get('no_mapeados', []), 1):
                print(f"      {idx}. {bio}")
            print(f"\n   ⚠️ El caso está INCOMPLETO - Requiere agregar biomarcadores al sistema\n")

        # 3.4.5 FORMATOS NARRATIVOS DE BIOMARCADORES (v6.1.7)
        print("Validando formatos de biomarcadores (POSITIVO/NEGATIVO estandarizado)...")
        estado_formatos = self._detectar_formatos_narrativos_biomarcadores(bd, criticos, ocr)
        auditoria['formatos_biomarcadores'] = estado_formatos
        if estado_formatos['estado'] == 'ERROR':
            auditoria['errores'].append(estado_formatos['mensaje'])
            auditoria['estado'] = 'ERROR'
        elif estado_formatos['estado'] == 'WARNING':
            auditoria['warnings'].append(estado_formatos['mensaje'])
    ...
    def _validar_biomarcadores_completos(self, bd: Dict, criticos: Dict, ocr: str) -> Dict:
        ...
        # V6.2.2: CRÍTICO - Caso INCOMPLETO si hay biomarcadores solicitados pero NO mapeados (IHQ251012)
        estado = 'OK'
        if faltantes_en_bd or no_mapeados:
            estado = 'INCOMPLETO'  # V6.2.2: Cambiado de ERROR a INCOMPLETO para distinguir
    ...
    def _detectar_formatos_narrativos_biomarcadores(self, bd: Dict, criticos: Dict, ocr: str) -> Dict:
        """
        Detecta biomarcadores con formatos narrativos no estandarizados.

        V6.1.7: Nueva función para detectar y sugerir normalizaciones automáticas.

        Formatos detectados:
        - "EXPRESIÓN DE X" → "POSITIVO"
        - "AUSENCIA DE EXPRESIÓN" → "NEGATIVO"
        - "EXPRESIÓN ABERRANTE" → "POSITIVO (aberrante)"
        - "RESTRICCIÓN DE CADENAS LIVIANAS" → "POSITIVO (restricción)"
        - Valores largos (>40 caracteres) que deberían ser POSITIVO/NEGATIVO
        ...
        Returns:
            Dict con biomarcadores detectados y sugerencias de normalización
        """
        import re

        problemas = []

        # Obtener todas las columnas IHQ_*
        columnas_ihq = {k: v for k, v in criticos.items() if k.startswith('IHQ_') and k != 'IHQ_ESTUDIOS_SOLICITADOS' and k != 'IHQ_ORGANO'}

        # Patrones de formatos narrativos problemáticos
        patrones_narrativos = [
            {
                'patron': r'^EXPRESI[OÓ]N\s+DE\s+',
                'tipo': 'EXPRESION_DE',
                'sugerencia_base': 'POSITIVO',
                'descripcion': 'Formato "EXPRESIÓN DE [biomarcador]" debería ser "POSITIVO"'
            },
            {
                'patron': r'^AUSENCIA\s+DE\s+EXPRESI[OÓ]N',
                'tipo': 'AUSENCIA_EXPRESION',
                'sugerencia_base': 'NEGATIVO',
                'descripcion': 'Formato "AUSENCIA DE EXPRESIÓN" debería ser "NEGATIVO"'
            },
            {
                'patron': r'EXPRESI[OÓ]N\s+ABERRANTE',
                'tipo': 'EXPRESION_ABERRANTE',
                'sugerencia_base': 'POSITIVO (aberrante)',
                'descripcion': 'Formato "EXPRESIÓN ABERRANTE" debería ser "POSITIVO (aberrante)"'
            },
            {
                'patron': r'RESTRICCI[OÓ]N\s+DE\s+CADENAS',
                'tipo': 'RESTRICCION_CADENAS',
                'sugerencia_base': 'POSITIVO (restricción)',
                'descripcion': 'Formato "RESTRICCIÓN DE CADENAS LIVIANAS" debería ser "POSITIVO (restricción)"'
            },
            {
                'patron': r'EXPRESI[OÓ]N\s+D[EÉ]BIL',
                'tipo': 'EXPRESION_DEBIL',
                'sugerencia_base': 'POSITIVO (débil)',
                'descripcion': 'Formato "EXPRESIÓN DÉBIL" debería ser "POSITIVO (débil)"'
            },
            {
                'patron': r'MARCACI[OÓ]N\s+(POSITIV|NEGATIV)',
                'tipo': 'MARCACION',
                'sugerencia_base': None,  # Depende del grupo capturado
                'descripcion': 'Formato "MARCACIÓN POSITIVA/NEGATIVA" debería ser "POSITIVO/NEGATIVO"'
            }
        ]

        for columna, valor in columnas_ihq.items():
            if not valor or not str(valor).strip():
                continue

            valor_str = str(valor).strip().upper()
            biomarcador = columna.replace('IHQ_', '').replace('_', '-')

            # Detectar formatos narrativos
            for patron_info in patrones_narrativos:
                match = re.search(patron_info['patron'], valor_str)
                if match:
                    # Generar sugerencia específica
                    if patron_info['tipo'] == 'MARCACION':
                        # Extraer si es POSITIVA o NEGATIVA
                        estado = 'POSITIVO' if 'POSITIV' in valor_str else 'NEGATIVO'
                        sugerencia = estado
                    else:
                        sugerencia = patron_info['sugerencia_base']
                    ...
                    problemas.append({
                        ...
                        'tipo_problema': patron_info['tipo'],
                        'descripcion': patron_info['descripcion'],
                        'longitud_valor': len(valor_str)
                    })
                    break  # Solo reportar el primer problema encontrado por biomarcador

            # Detectar valores excesivamente largos (>40 caracteres) sin match de patrón específico
            if len(valor_str) > 40 and not any(p['columna'] == columna for p in problemas):
                # Inferir POSITIVO/NEGATIVO basado en keywords
                if 'POSITIV' in valor_str or 'EXPRESI' in valor_str:
                    sugerencia = 'POSITIVO'
                elif 'NEGATIV' in valor_str or 'AUSENCIA' in valor_str or 'SIN' in valor_str:
                    sugerencia = 'NEGATIVO'
                else:
                ...
                problemas.append({
                    ...
                    'valor_actual': valor,
                    'valor_sugerido': sugerencia,
                    'tipo_problema': 'VALOR_LARGO',
                    'descripcion': f'Valor muy largo ({len(valor_str)} caracteres) debería estandarizarse',
                    'longitud_valor': len(valor_str)
                })

        # Calcular estadísticas
        total_biomarcadores = len(columnas_ihq)
        biomarcadores_con_problemas = len(problemas)
        porcentaje_problemas = (biomarcadores_con_problemas / total_biomarcadores * 100) if total_biomarcadores > 0 else 0

        estado = 'OK'
        if biomarcadores_con_problemas > 0:
            if porcentaje_problemas > 50:
                estado = 'ERROR'
            else:
                estado = 'WARNING'
        ...
        return {
            ...
            'biomarcadores_con_problemas': biomarcadores_con_problemas,
            'porcentaje_problemas': round(porcentaje_problemas, 1),
            'problemas': problemas,
            'mensaje': f'{biomarcadores_con_problemas}/{total_biomarcadores} biomarcadores con formato narrativo no estandarizado' if problemas else 'Todos los biomarcadores tienen formato estandarizado'
        }
    ...
    def _validar_malignidad(self, campos_criticos: Dict, ocr: str) -> Dict:
        ...
        keywords_maligno = [
            ...
            'TIMOMA', 'TIMOMA AB', 'TIMOMA TIPO B'  # V6.2.2: Tumores del timo (IHQ251012)
        ]
        keywords_benigno = [
    ...
    def _calcular_metricas_finales(self, resultado: Dict) -> None:
        ...
        # V6.2.2: Determinar estado final considerando INCOMPLETO (biomarcadores no mapeados)
        # Prioridad: INCOMPLETO > ERROR > ADVERTENCIA > OK
        biomarcadores_estado = auditoria_bd.get('biomarcadores', {}).get('estado')

        if biomarcadores_estado == 'INCOMPLETO':
            # Caso tiene biomarcadores solicitados pero NO mapeados en el sistema
            estado = 'INCOMPLETO'
        elif auditoria_bd.get('errores'):
            estado = 'ERROR'
        ...
        # V6.2.2: Agregar info de biomarcadores no mapeados a métricas
        biomarcadores_info = auditoria_bd.get('biomarcadores', {})
        no_mapeados_count = len(biomarcadores_info.get('no_mapeados', []))
        total_solicitado = biomarcadores_info.get('total_solicitado_ocr', 0)
        mapeados_count = biomarcadores_info.get('mapeados', 0)
        ...
        resultado['metricas'] = {
            ...
            # V6.2.2: Métricas de completitud de biomarcadores
            'biomarcadores_solicitados': total_solicitado,
            'biomarcadores_mapeados': mapeados_count,
            'biomarcadores_no_mapeados': no_mapeados_count,
            'completitud_biomarcadores': round((mapeados_count / total_solicitado * 100) if total_solicitado > 0 else 100, 1)
        }
        ...
        # V6.2.2: Mostrar info de completitud de biomarcadores
        if total_solicitado > 0:
            print(f"Biomarcadores mapeados: {mapeados_count}/{total_solicitado} ({resultado['metricas']['completitud_biomarcadores']}%)")
            if no_mapeados_count > 0:
                print(f"⚠️ {no_mapeados_count} biomarcadores NO mapeados en el sistema")
        print()
