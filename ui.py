...
class App(ttk.Window):
    ...
    def _create_floating_button(self):
        ...
        self.floating_btn_container = ttk.Frame(
            self,
            relief="flat",   # V6.9.44: sin borde "raised" -> elimina el recuadro blanco
            borderwidth=0
        )
        ...
        self.floating_btn = ttk.Button(
            ...
            command=self._toggle_floating_menu,
            # V6.9.44: estilo SÓLIDO (no "-outline"): el outline tiene relleno
            # blanco en reposo que tapaba el fondo. Sólido = FAB azul intencional.
            bootstyle="info",
            width=2,  # Más compacto
            cursor="hand2"
        )
        # Sin padx/pady: el botón cubre todo el contenedor -> no asoma fondo blanco
        ...
        try:
            ...
            if peticion_col_idx is not None:
                try:
                    ...
                    # V6.9.44 FIX: completitud sobre DATOS EN VIVO (self.master_df, BD
                    # configurada -> MySQL). Antes verificar() releia un SQLite local
                    # (DB_FILE) desincronizado tras reprocesar -> "no encontrado" ->
                    # incompletos -> TODA la tabla en ROJO. master_df ya esta en memoria.
                    registros_por_caso = {}
...
Informes con malignidad: {malignant_count}"""
    ...
    _PROMPT_SYSTEM_IA_OCR = (
        "Eres analista patológico. Recibirás texto OCR de UN SOLO informe IHQ.\n"
        "Tu tarea es extraer 3 datos: numero_peticion, diagnostico, organo.\n\n"
        "INSTRUCCIONES:\n\n"
        "1) numero_peticion: copiá el código 'IHQXXXXXX' que aparece en\n"
        "   'N. peticion : IHQXXXXXX'. Solo el código, formato 'IHQ'+dígitos.\n\n"
        "2) diagnostico: la entidad clínica COMPLETA de la sección DIAGNÓSTICO.\n"
        "   La sección DIAGNÓSTICO suele estar al FINAL del informe. Empieza\n"
        "   con una frase tipo 'Órgano. Biopsia. Estudio IHQ:' seguida de la\n"
        "   entidad clínica real (en bullets, líneas o párrafo).\n"
        "   Devolvé la ENTIDAD COMPLETA, NO la frase introductoria.\n\n"
        "   IMPORTANTE — NO trunques. Capturá la primera línea sustantiva\n"
        "   COMPLETA, incluyendo calificadores adicionales que enriquecen el\n"
        "   diagnóstico ('CON DATOS DE...', 'WHO GRADO X', '(g2, ptc3, v0)',\n"
        "   'p40 POSITIVO', etc.). Solo descartá las descripciones que están\n"
        "   en bullets SEPARADOS y que SON otros sub-items distintos del dx\n"
        "   principal (PATRÓN MICROSATELITAL, biomarcadores como HER-2: NEG).\n\n"
        "   Ejemplos:\n"
        "     PDF: 'DIAGNÓSTICO\n"
        "           Mucosa de estomago. Biopsia. Estudio de IHQ:\n"
        "           - CARCINOMA POCO COHESIVO\n"
        "           - PATRÓN MICROSATELITAL ESTABLE'\n"
        "     → diagnostico = 'CARCINOMA POCO COHESIVO'\n"
        "     (la primera entidad; el patrón microsatelital es sub-info aparte)\n\n"
        "     PDF: 'DIAGNÓSTICO\n"
        "           Riñón. Biopsia. Estudio de IHQ:\n"
        "           - RECHAZO ACTIVO CON DATOS SUGERENTES DE COMPONENTE HUMORAL\n"
        "             (g2, ptc3, v0)'\n"
        "     → diagnostico = 'RECHAZO ACTIVO CON DATOS SUGERENTES DE COMPONENTE HUMORAL (g2, ptc3, v0)'\n"
        "     (línea completa con scoring; NO solo 'RECHAZO ACTIVO')\n\n"
        "     PDF: 'DIAGNÓSTICO\n"
        "           Cerebro. Resección. Estudio IHQ:\n"
        "           - MENINGIOMA MENINGOTELIAL, WHO GRADO 1'\n"
        "     → diagnostico = 'MENINGIOMA MENINGOTELIAL, WHO GRADO 1'\n"
        "     (CON el grado WHO; NO solo 'MENINGIOMA')\n\n"
        "   Si el patólogo escribió texto descriptivo en lugar de dx claro\n"
        "   (ej: 'TÉJIDO SIN REPRESENTACIÓN DE PARENQUIMA RENAL'), copialo\n"
        "   COMPLETO, NO lo abrevies a 'VER COMENTARIO' o similar.\n\n"
        "3) organo: SOLO un órgano anatómico en MAYÚSCULAS, COPIADO de la\n"
        "   columna 'Organo' del header de Estudios solicitados.\n"
        ...
        "4) Copiá las palabras EXACTAS del PDF. NO inventes letras.\n"
        "   Si el patólogo escribió 'CARICNOMA' (typo), copiá 'CARICNOMA'.\n\n"
        "5) Si no encontrás dx claro porque el patólogo redirige al lector\n"
        "   (ej: 'VER DESCRIPCIÓN MICROSCÓPICA Y COMENTARIO'), copiá esa\n"
        "   frase tal cual como diagnostico.\n\n"
        "FORMATO JSON OBLIGATORIO — UN SOLO informe en el array:\n"
        '{"diagnosticos":[\n'
    ...
    def _process_selected_files_ia(self):
        ...
        respuesta = messagebox.askyesno(
            ...
            f"  2. El texto se envía al LLM local (Ollama o LM Studio) o proveedor cloud\n"
    ...
    def _process_files_ia_worker(self, files_to_process, out_dir):
        ...
        for idx, (pdf_path, filename) in enumerate(files_to_process):
            ...
            def _llamar_llm_con_retry(chunk_text, intento_max_tokens=800):
                last_error = None
                for intento in (1, 2):
                    try:
                        max_tok = intento_max_tokens if intento == 1 else 1500
